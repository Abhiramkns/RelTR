...
class RelTR(nn.Module):
    ...
    def __init__(
        ...
        self.rel_class_embed = MLP(
            hidden_dim * 2 + 128, hidden_dim, num_rel_classes + 1, 2
        )
...
class SetCriterion(nn.Module):
    ...
    def __init__(
        self, num_classes, num_rel_classes, matcher, weight_dict, eos_coef, losses
    ):
        ...
        self.num_rel_classes = (
            51 if num_classes == 151 else 31
        )  # Using entity class numbers to adapt rel class numbers
    ...
    def ltn_predicate(self, pred, target):
            """Return probabilites of target classes
            """
            target_indices = torch.zeros(pred.shape, device=pred.device)
            target_indices.scatter_(
                len(pred.shape) - 1, target.unsqueeze(len(pred.shape) - 1), 1
            )

            softmax = nn.Softmax(dim=len(pred.shape) - 1)
            pred = softmax(pred)
            out = torch.sum(pred * target_indices, dim=len(pred.shape) - 1)
            return out
    ...
    def loss_labels(self, outputs, targets, indices, num_boxes, log=True):
        ...
        target_rels_classes_o = torch.cat(
            [
                t["labels"][t["rel_annotations"][J, 0]]
                for t, (_, J) in zip(targets, indices[1])
            ]
        )
        target_relo_classes_o = torch.cat(
            [
                t["labels"][t["rel_annotations"][J, 1]]
                for t, (_, J) in zip(targets, indices[1])
            ]
        )
        ...
        target_classes = torch.cat(
            (target_classes, target_sub_classes, target_obj_classes), dim=1
        )
        ...
        loss_weight = torch.cat(
            (
                torch.ones(pred_logits.shape[:2]).to(pred_logits.device),
                indices[2] * 0.5,
                indices[3] * 0.5,
            ),
            dim=-1,
        ...
        losses = {
            "loss_ce": (loss_ce * loss_weight).sum()
            / self.empty_weight[target_classes].sum()
        }

        if log:
            # TODO this should probably be a separate loss, not hacked in this one here
            losses["class_error"] = (
                100 - accuracy(pred_logits[idx], target_classes_o)[0]
            )
            losses["sub_error"] = (
                100 - accuracy(sub_logits[rel_idx], target_rels_classes_o)[0]
            )
            losses["obj_error"] = (
                100 - accuracy(obj_logits[rel_idx], target_relo_classes_o)[0]
            )
        return losses
    ...
    def loss_cardinality(self, outputs, targets, indices, num_boxes):
        ...
        tgt_lengths = torch.as_tensor(
            [len(v["rel_annotations"]) for v in targets], device=device
        )
    ...
    def loss_boxes(self, outputs, targets, indices, num_boxes):
        ...
        target_entry_boxes = torch.cat(
            [t["boxes"][i] for t, (_, i) in zip(targets, indices[0])], dim=0
        )
        ...
        target_boxes = torch.cat(
            (target_entry_boxes, target_rels_boxes, target_relo_boxes), dim=0
        )
    ...
    def loss_relations(self, outputs, targets, indices, num_boxes, log=True):
        ...
        loss_ce = F.cross_entropy(
            src_logits.transpose(1, 2), target_classes, self.empty_weight_rel
        )
    ...
    def loss_sat(self, outputs, targets, indices, num_boxes, log=True):
        """Compute the sat loss of relationship classifications"""
        # sub and obj source and target logits
        sub_logits = outputs["sub_logits"]
        obj_logits = outputs["obj_logits"]
        ...
        # rel source and target logits
        rel_src_logits = outputs["rel_logits"]
        target_rel_classes_o = torch.cat(
            [t["rel_annotations"][J, 2] for t, (_, J) in zip(targets, indices[1])]
        )
        target_rel_classes = torch.full(
            rel_src_logits.shape[:2],
            self.num_rel_classes,
            dtype=torch.int64,
            device=rel_src_logits.device,
        )
        target_rel_classes[rel_idx] = target_rel_classes_o
        
        sub_probs = self.ltn_predicate(sub_logits, target_sub_classes).flatten()
        obj_probs = self.ltn_predicate(obj_logits, target_obj_classes).flatten()
        rel_probs = self.ltn_predicate(rel_src_logits, target_rel_classes).flatten()
        
        l = sub_probs.shape[0]
        axioms = []
        for i in range(l):
            sub = ltn.Constant(sub_probs[i])
            obj = ltn.Constant(obj_probs[i])
            rel = ltn.Constant(rel_probs[i])
            axioms.append(self.SatAgg(self.And(self.And(sub, rel), obj)))
        sat_agg = self.SatAgg(*axioms)
        losses = {"loss_sat": 1.0 - sat_agg}
        return losses
...
def build(args):
    num_classes = (
        151 if args.dataset != "oi" else 289
    ...
    weight_dict["loss_sat"] = args.rel_sat_loss_coef
    ...
    losses = ["labels", "boxes", "cardinality", "relations", "sat"]
